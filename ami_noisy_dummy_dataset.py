...
class AMI_noisy_dummy_dataset(torch.utils.data.Dataset):
	...
	def _load_noisy_dummy_data(self):
		'''Loads the data from the file into the data object'''
		...
		self.keys,self.matrices = self._filter_on_frequency_bounds(self.keys,self.matrices,frequency_bounds = self.frequency_bounds)
		self.keys,self.matrices = self._keep_top_n_frequent_keys(self.keys, self.matrices, n = 100)
	...
	def _keep_top_n_frequent_keys(self,keys,matrices,n):
