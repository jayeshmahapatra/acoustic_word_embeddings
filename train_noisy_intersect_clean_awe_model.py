...
from ami_noisy_intersect_clean_dataset import AMI_noisy_intersect_clean_dataset
...
if __name__ == '__main__':
	...
	train_ds = AMI_noisy_intersect_clean_dataset(num_examples = num_examples, split_set = "train", data_filepath = data_filepath, char_threshold = 5, frequency_bounds = (0,np.Inf))
	train_dl = DataLoader(train_ds, batch_size=bs, pin_memory = True, shuffle = True, drop_last = True)

	val_ds = AMI_noisy_intersect_clean_dataset(num_examples = num_examples, split_set = "val", data_filepath = data_filepath, char_threshold = 5, frequency_bounds = (0,np.Inf))
	...
	hist = train_model(net,num_epochs,train_dl,val_dl,optimizer,criterion,dev,save_path="/data/users/jmahapatra/models/noisy_interesect_clean/",verbose = True)
	#hist = train_model(net,num_epochs,train_dl,val_dl,optimizer,criterion,dev,save_path="./Models/test/",verbose = True)
	
	plot_learning_curves(hist,'/data/users/jmahapatra/data/noisy_interesect_clean_learning_curves.png', show = False)
